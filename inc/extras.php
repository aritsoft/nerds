...
if ( ! function_exists( 'adammp_breadcrumbs' ) ) {
	...
	function adammp_breadcrumbs( $args = array() ) {
		...
		$html .= '<li class="lnk_pag link-home"><a class="bread-link bread-home" href="' . esc_url( get_home_url() ) . '" title="' . esc_attr( $args['home_title'] ) . '">' . esc_html( $args['home_title'], 'nerds' ) . '</a></li>';

		// Post

		if ( is_singular( 'post' ) ) {

			$html .= '<li class="lnk_pag item-current item-' . $post->ID . '"><span class="bread-current bread-' . $post->ID . '" title="' . esc_attr( get_the_title() ) . '">' . esc_html( get_the_title(), 'nerds' ) . '</span></li>';
			...
			if ( $post->post_parent ) {
				...
				foreach ( $parents as $parent ) {
					...
					$html .= '<li class="lnk_pag item-parent item-parent-' . esc_attr( $parent ) . '"><a class="bread-parent bread-parent-' . esc_attr( $parent ) . '" href="' . esc_url( get_permalink( $parent ) ) . '" title="' . esc_attr( get_the_title( $parent ) ) . '">' . esc_html( get_the_title( $parent ), 'nerds' ) . '</a></li>';

				}

			}

			$html .= '<li class="lnk_pag item-current item-' . $post->ID . '"><span title="' . esc_attr ( get_the_title() ) . '"> ' . esc_html( get_the_title(), 'nerds' ) . '</span></li>';
			...
			$html .= '<li class="lnk_pag item-parent"><a class="bread-parent" href="' . esc_url( $parent_permalink ) . '" title="' . esc_attr( $parent_title ) . '">' . esc_html( $parent_title, 'nerds' ) . '</a></li>';
			...
			$html .= '<li class="lnk_pag item-cat item-custom-post-type-' . esc_attr( $post_type ) . '"><a class="bread-cat bread-custom-post-type-' . esc_attr( $post_type ) . '" href="' . esc_url( $post_type_archive ) . '" title="' . esc_attr( $post_type_object->labels->name ) . '">' . esc_html( $post_type_object->labels->name , 'nerds' ) . '</a></li>';

			$html .= '<li class="lnk_pag item-current item-' . $post->ID . '"><span class="bread-current bread-' . $post->ID . '" title="' . esc_attr( $post->post_title ) . '">' . esc_html ($post->post_title, 'nerds' ) . '</span></li>';
			...
			if ( $parent !== 0 ) {
				...
				$html .= '<li class="lnk_pag item-parent item-parent-' . esc_attr( $parent_category->slug ) . '"><a class="bread-parent bread-parent-' . esc_attr( $parent_category->slug ) . '" href="' . esc_url( $category_link ) . '" title="' . esc_attr( $parent_category->name ) . '">' . esc_html( $parent_category->name, 'nerds' ) . '</a></li>';
			...
			$html .= '<li class="lnk_pag">' . esc_html__( 'Error 404', 'nerds' ) . '</li>';
